...
class Follows {
  ...
  static followPlugin = (callback) => {
    if (!authManager.currentUser)
      return callback("Must be logged in before following a plugin");
        ...
        buildfire.appData.search(
          ...
          (e, r) => {
            if (e || !r) return callback(e);
              ...
              if (
                r[0].data.followedPlugins.findIndex((e) => e == pluginId) >= 0
              )
                return callback("Already following this plugin");
              buildfire.appData.update(
                r[0].id,
                {
                  ...r[0].data,
                  followedPlugins: [...r[0].data.followedPlugins, pluginId],
                },
                Follows.TAG,
                (e, r) => {
                  if (e) return callback(e);
  ...
  static unfollowPlugin = (callback) => {
    if (!authManager.currentUser)
      return callback("Must be logged in before unfollowing a plugin");
    buildfire.getContext((err, context) => {
      ...
      buildfire.appData.search(
        ...
        (e, r) => {
          if (e || !r) return callback(e);
            ...
            return callback("You are not following this plugin");
          else {
            let obj = { ...r[0].data };
  ...
  static toggleFollowPlugin = (callback) => {
    if (!authManager.currentUser)
      return callback("Must be logged in before following a user");
    buildfire.getContext((err, context) => {
      if (err) return callback(err);
      else {
        let pluginId = context.instanceId;
  ...
  static followUser = (fUserId, callback) => {
    ...
    if (fUserId == authManager.currentUser._id)
      return callback("You can't follow yourself");
    buildfire.appData.search(
      { filter: { "_buildfire.index.string1": authManager.currentUser._id } },
      Follows.TAG,
      (e, r) => {
        ...
        buildfire.auth.getUserProfile({ userId: fUserId }, (e, u) => {
          ...
          if (r.length == 0) {
            buildfire.appData.insert(
              Follows.createFollowData(authManager.currentUser, fUserId),
              Follows.TAG,
              (e, r) => {
            ...
            buildfire.appData.update(
              ...
              {
                ...r[0].data,
                followedUsers: [...r[0].data.followedUsers, fUserId],
              },
              Follows.TAG,
              (e, r) => {
                if (e) return callback(e);
  ...
  static unfollowUser = (fUserId, callback) => {
    ...
    if (fUserId == authManager.currentUser._id)
      return callback("You can't unfollow yourself");
    buildfire.appData.search(
      { filter: { "_buildfire.index.string1": authManager.currentUser._id } },
      Follows.TAG,
      (e, r) => {
        ...
        buildfire.auth.getUserProfile({ userId: fUserId }, (e, u) => {
          if (e) return callback(e);
            ...
            return callback("You are not following this user");
          else {
            let obj = { ...r[0].data };
  ...
  static isFollowingUser = (userId, callback) => {
    ...
    if (userId == authManager.currentUser._id)
      return callback("You can't be following yourself");
