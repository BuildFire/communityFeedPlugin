...
const createImage = (src, loadInPreviewer = true) => {
  ...
  if (loadInPreviewer) {
    image.onclick = () => {
      ...
      buildfire.imagePreviewer.show({ images: ["https://images.unsplash.com/photo-1477959858617-67f85cf4f1df?ixlib=rb-1.2.1&ixid=MnwxMjA3fDB8MHxzZWFyY2h8Mnx8Y2l0eXxlbnwwfHwwfHw%3D&w=1000&q=80"] }, () =>
...
const createPostBody = (post, postId, postContainer) => {
  ...
  if (post?.postText) {
    ...
    if (post.postText.length > length) {
      let newText = post?.postText.substring(0, length) + "..."; // trimmed post
      postDescription = createElement(
        "h3",
        newText,
        ["publicPostText", "inlineBlock"],
        `${postId}postDescription`
      );
      postContainer.appendChild(postDescription);
      let postTextSeeMore = createElement(
        "h3",
        "See More",
        ["publicPostText", "seeMore", "inlineBlock", "bold"],
        `${postId}postDescription`
      );
      postContainer.appendChild(postTextSeeMore);
      let postFullDescription = createElement(
        "h3",
        post.postText,
        ["publicPostText", "inlineBlock", "remove"],
        `${postId}postDescription`
      );
  ...
  if (post?.postImages) {
    let postImagesContainer = createElement(
      "div",
      "",
      ["publicPostImages"],
      `${postId}postImagesContainer`
    );
    if (post.postImages.length > 3) {
      ...
      let remainingImages = createElement(
        "div",
        "",
        ["remainingImages"],
        `${postId}postRemainingImages`
      );
      ...
      let seeMoreButton = createElement(
        "div",
        "",
        ["seeMoreButton"],
        `${postId}ShowMoreContainer`
      );
      let button = createElement(
        "button",
        `Show ${post.postImages.length - 3} More`,
        null,
        `${postId}ShowMoreButton`
      );
      button.onclick = () => {
        document.getElementById(`${postId}postRemainingImages`).style.display =
          "block";
        document.getElementById(`${postId}ShowMoreContainer`).style.display =
          "none";
      };
...
const createPublicPostUI = (post, parentId, postId) => {
  ...
  let postContainer = createElement(
    "div",
    "",
    ["publicPost"],
    `${postId}publicPost`
  );
  let postTitle = createElement(
    "h2",
...
const getPostTime = (time) => {
  ...
  if (parseInt(diffInDays) > 0) finalDiff = diffInDays + " days";
  else if (parseInt(diffInHours) > 0) finalDiff = diffInHours + " hrs";
  else if (parseInt(diffInMins) > 0) finalDiff = diffInMins + " mins";
  return finalDiff;
};
...
const createUserPost = (post, parentId, postId) => {
  ...
  let profileSection = createElement(
    "div",
    "",
    ["profileSection"],
    `${postId}ProfileSection`
  );
  let profileImageContainer = createElement(
    "div",
    "",
    ["profileImageContainer"],
    `${postId}profileImageContainer`
  );
  ...
  let postPluginSectionInfo = createElement("div", "", [
    "postPluginSectionInfo",
  ]);

  let pluginTitle = createElement(
    "h3",
    post.pluginInstance.pluginInstanceTitle,
    ["pluginTitle"],
    `${postId}pluginTitle`
  ...
  pluginTitle.onclick = () => {
    console.log(pluginTitle.getAttribute("instanceId"));
    buildfire.navigation.navigateTo({
      instanceId: pluginTitle.getAttribute("instanceId"),
    });
  };
...
const showMoreImages = (postId) => {
  let post = document.getElementById("remainingImages1");
  post.style.display = "block";
  let button = document.getElementById("showMore1");
